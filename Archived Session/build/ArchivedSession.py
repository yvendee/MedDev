...
class Table(tk.Frame):
    ...
    def create_table(self):
        headers = ['Date', 'Hand', 'Thumb', 'Pointer' , 'Middle', 'Ring', 'Pinky']
        self.data = mydata
        # self.data = [
        ...
        for col, header in enumerate(headers):
            header_label = tk.Label(self.scrollable_frame, text=header, font=('Arial', 16, 'bold'))
            header_label.grid(row=0, column=col, padx=10, pady=5)

        # Create data rows
        for row, entry in enumerate(self.data, start=1):
            for col, value in enumerate(entry):
                data_label = tk.Label(self.scrollable_frame, text=value, font=('Arial', 16))
